...
TEMPLATE_MODULE_DIR: Path = PROJECT_ROOT  # this repo
...
def generate_cmd(
    ...
    for ex in to_render:
        ...
        # ───── 2. Run the *module* template (child) ───────────────────────────
        module_test_dir = ex_dir / "module_run"
        module_test_dir.mkdir()
        c_module = _new_copie_instance(
            template_dir=TEMPLATE_MODULE_DIR,
            test_dir=module_test_dir,
            config_file=config_file,
            parent_result=pkg_result,
        )
        if ex.module_answers is None:
            typer.echo(
                f"[{ex.name}] No module answers found, skipping module template.",
                err=True,
            )
            continue
        module_result = c_module.copy(extra_answers=ex.module_answers)

        if module_result.exception or module_result.exit_code != 0:  # pragma: no cover
            typer.echo(
                f"[{ex.name}] Module template failed: {module_result.exception}",
                err=True,
            )
            continue
        ...
        typer.secho(
            ...
            f"    {module_result.project_dir}",  # normally <sandbox>/<name>/module_run/copie000/…
            fg="green",
        )
